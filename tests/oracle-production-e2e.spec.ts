...
test.describe('Oracle Production E2E Test - HubManager → SalesHub', () => {
  test.setTimeout(180000); // 3분 타임아웃

  test('Google OAuth 로그인 및 SalesHub 접근 테스트', async ({ page }) => {
    console.log('🚀 Starting Oracle Production E2E Test...');
    ...
    page.on('response', async (response) => {
      ...
      if (url.includes('/api/auth/') || url.includes('google-callback')) {
