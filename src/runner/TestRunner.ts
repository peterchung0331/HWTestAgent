...
import { AutoFixer } from '../ai/AutoFixer.js';
      ...
      for (let i = 0; i < scenario.steps.length; i++) {
        ...
        if (stepResult.status === 'FAILED' && (options.auto_fix ?? true)) {
          ...
          for (let attempt = 1; attempt <= maxRetry; attempt++) {
            ...
            // Analyze failure and get fix suggestion
            const fixSuggestion = await this.autoFixer.analyzeFix(
              step,
              stepResult,
              options.project,
              options.scenario
            ...
            console.log(`   💡 Suggestion: ${fixSuggestion.reason} (confidence: ${(fixSuggestion.confidence * 100).toFixed(0)}%)`);

            if (fixSuggestion.type === 'alert') {
              console.log(`   ⚠️  Manual intervention required - stopping retries`);
              break;
            }

            if (fixSuggestion.type === 'skip') {
              console.log(`   ⏭️  Skipping step as suggested`);
              stepResult.status = 'SKIPPED';
              break;
            }

            // Apply fix if confidence is high enough
            const modifiedStep = this.autoFixer.shouldApplyFix(fixSuggestion)
              ? this.autoFixer.applyFix(step, fixSuggestion)
              : step;

            // Wait before retry (longer for rate limits)
            const waitTime = stepResult.response_data?.status === 429 ? 5000 : 2000;
            await this.wait(waitTime);

            stepResult = await this.httpAdapter.executeStep(modifiedStep);
            retry_count++;
